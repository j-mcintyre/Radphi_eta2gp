...
void Prompted_Mass_wgt_fit() {
   ...
   */
   ...
   TFile *fStep3 = new TFile("/nfs/direct/annex/mcintyre/GitRepo/Radphi_eta2gp/RUN0/STEP3/indiv_fits/TwoG_may_14_2023_STEP3.root");

   // Total number of bins (e.g. weights to import)
   int totalBins = NumEChan * tBinNum;
   double X_eta_sig_t[totalBins];
   double X_err_sig_t[totalBins];
   double X_eta_sb1_t[totalBins];
   double X_err_sb1_t[totalBins];
   double X_eta_sb2_t[totalBins];
   double X_err_sb2_t[totalBins];

   double eta_sig_t[tBinNum];
   double err_sig_t[tBinNum];
   double eta_sb1_t[tBinNum];
   double err_sb1_t[tBinNum];
   double eta_sb2_t[tBinNum];
   ...
   for (int i = 0; i < NumEChan; i++) {
      ...
      for (int j = 0; j < tBinNum; j++) {
         ...
         int bbq = 2 * j + 1;
	    	int acq = bbq + 1;
         X_eta_sig_t[WeightCount] = h_W0->GetBinContent(bbq);
         X_err_sig_t[WeightCount] = h_W0->GetBinContent(acq);

         X_eta_sb1_t[WeightCount] = h_W1->GetBinContent(bbq);
         X_err_sb1_t[WeightCount] = h_W1->GetBinContent(acq);

         X_eta_sb2_t[WeightCount] = h_W2->GetBinContent(bbq);
   ...
   // Create a ROOT file for further analysis
  	TFile *MyFile = new TFile("TwoG_may_14_2023_STEP4.root", "NEW");
   MyFile->Close();
   ...
   for(int Echannel = 0; Echannel < NumEChan; Echannel++){
      ...
      int NodeCount = 0;
		for(int i = 0; i < tBinNum; i++) {
         ...
         if (((Echannel == 0) && (i == 0 || i == 7 || i == 10 || i == 13 || i == 17 || i == 19)) ||
             ((Echannel == 1) && (i == 0 || i == 3 || i == 5 || i == 11 || i == 16 || i == 19)) ||
				 ((Echannel == 2) && (i == 0 || i == 1 || i == 3 || i == 10 || i == 12 || i == 16 || i == 19)) ||
				 ((Echannel == 3) && (i == 0 || i == 4 || i == 11 || i == 15 || i == 19))) {
				double ZZ = tbins[i][1] - tbins[i][0];
				PARTmeson_X[mi] = tbins[i][0] + (ZZ / 2.0);

            // Get a subset of bins to fit with a TSpline
				w_sig_t[mi] = X_eta_sig_t[HistBinCount];
				w_err_sig_t[mi] = X_err_sig_t[HistBinCount];

				w_sb1_t[mi]  = X_eta_sb1_t[HistBinCount];
				w_err_sb1_t[mi] = X_err_sb1_t[HistBinCount];

				w_sb2_t[mi]  = X_eta_sb2_t[HistBinCount];
      ...
      TFile *MyFile = new TFile("TwoG_may_14_2023_STEP4.root", "UPDATE");
      ...
      for (int wrt = 0; wrt < tBinNum; wrt++) {
         ...
         eta_sig_t[wrt] = X_eta_sig_t[placeholder];
         err_sig_t[wrt] = X_err_sig_t[placeholder];
         eta_sb1_t[wrt] = X_eta_sb1_t[placeholder];
         err_sb1_t[wrt] = X_err_sb1_t[placeholder];
         eta_sb2_t[wrt] = X_eta_sb2_t[placeholder];
         err_sb2_t[wrt] = X_err_sb2_t[placeholder];
      }

      int fullN = tBinNum;
		c103->cd();
		TGraphErrors *gr3 = new TGraphErrors(fullN, meson_X, eta_sig_t, ex, err_sig_t);
		gr3->SetTitle("#eta Signal Region Weights;|t| [GeV^2/c^{4}];Fill Weight");
		gr3->GetYaxis()->CenterTitle(true);
		gr3->Draw("ALP");

		hSigpng.Form("Eta_w0_E%d.png", Echannel);
      c103->Print(hSigpng);

      s->Draw("same");

		hSig.Form("Eta_w0_spline_E%d.png", Echannel);
      c103->Print(hSig);

		gFile = MyFile;
		hSB.Form("Eta_w0_E%d", Echannel);
		gDirectory->WriteObject(gr3, hSB);


		c102->cd();
		TGraphErrors *gr2 = new TGraphErrors(fullN,meson_X,eta_sb1_t,ex,err_sb1_t);
		gr2->SetTitle("#eta SB Region 1 Weights (SB1);|t| [GeV^2/c^{4}];Fill Weight");
		gr2->GetYaxis()->CenterTitle(true);
		gr2->Draw("ALP");

		hSigpng.Form("Eta_w1_E%d.png", Echannel);
      c102->Print(hSigpng);

		st->Draw("same");

		hSig.Form("Eta_w1_spline_E%d.png", Echannel);
      c102->Print(hSig);

		gFile = MyFile;
		hSB.Form("Eta_w1_E%d", Echannel);
		gDirectory->WriteObject(gr2, hSB);


		c107->cd();
		TGraphErrors *gr4 = new TGraphErrors(fullN, meson_X, eta_sb2_t, ex, err_sb2_t);
		gr4->SetTitle("#eta SB Region 2 Weights (SB2);|t| [GeV^2/c^{4}];Fill Weight");
		gr4->GetYaxis()->CenterTitle(true);
		gr4->Draw("ALP");

		hSigpng.Form("Eta_w2_E%d.png", Echannel);
      c107->Print(hSigpng);

		st2->Draw("same");

		hSig.Form("Eta_w2_spline_E%d.png", Echannel);
      c107->Print(hSig);

		gFile = MyFile;
		hSB.Form("Eta_w2_E%d", Echannel);
