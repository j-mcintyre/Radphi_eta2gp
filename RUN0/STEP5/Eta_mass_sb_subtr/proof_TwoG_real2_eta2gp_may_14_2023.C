...
//    This files name  --> proof_TwoG_real2_eta2gp_may_14_2023
//    TSelector Name   --> TwoG_real2_eta2gp_may_14_2023
//
//   ///////////////// For MC Data ///////////////////
...
// --> TwoG_real2_eta2gp_may_14_2023Analysis.root
// --------------------------------------------------------
...
TChain *fullchain()
{
   ...
   while (const char *fname = gSystem->GetDirEntry(datadir)) {
      ...
      //if ((fnames.find("eta2gp_gen") != fnames.npos) && (fnames.find(".root") != fnames.npos)) {  // For MC data
   ...
   //std::cout << "There are ==> " << filecount << " Monte Carlo files with name(s) - " << "eta2gp_gen" << " - being processed. \n" ;
   gSystem->FreeDirectory(datadir);
   return ch;
}


void proof_TwoG_real2_eta2gp_may_14_2023(int nevents=2000000000)
{
   TChain *ch=fullchain();
   ...
   gProof->Load("TwoG_real2_eta2gp_may_14_2023.C+O");
   ch->SetProof();
#else
   gROOT->ProcessLine(".L TwoG_real2_eta2gp_may_14_2023.C+O");
#endif

   ch->Process("TwoG_real2_eta2gp_may_14_2023", "", nevents);
