#define TwoG_real2_eta2gp_may_14_2023_cxx
...
// __________________________________________________________________ //
//                                                                    //
// This file is a re-do of ER_real_eta2gp_oct_21_2021.C,              //
...
#include "TwoG_real2_eta2gp_may_14_2023.h"
#include <iostream>
...
void TwoG_real2_eta2gp_may_14_2023::Begin(TTree * /*tree*/)
{
   TString option = GetOption();
}

void TwoG_real2_eta2gp_may_14_2023::SlaveBegin(TTree * /*tree*/)
{
   TString option = GetOption();

   // Get previous analysis procedure values from STEP2 root file
   TFile *rootf = new TFile("/nfs/direct/annex/mcintyre/GitRepo/Radphi_eta2gp/RUN0/STEP2/total_fit/TwoG_may_14_2023_STEP2.root");
   ...
   TFile *rootS4f = new TFile("/nfs/direct/annex/mcintyre/GitRepo/Radphi_eta2gp/RUN0/STEP4/Eta_mass_SBwgt/TwoG_may_14_2023_STEP4.root");
   ...
   tEta0 = new TH1D("tEta0", "|t| from E_{#gamma} = 5.115 to 5.385 GeV for #eta;|t| [GeV^{2}/c^{4]];Events per 5 MeV^{2}", 380, 0.0, 1.9);
   tEta0->Sumw2();
   GetOutputList()->Add(tEta0);

   dphi_Eta0 = new TH1D("dphi_Eta0", "#Delta#phi from E_{#gamma} = 5.115 to 5.385 GeV for #eta all |t|;#Delta#phi [degree];Events per 2 deg.", 180, -180.0, 180.0);
   dphi_Eta0->Sumw2();
   GetOutputList()->Add(dphi_Eta0);

   tEta1 = new TH1D("tEta1", "|t| from E_{#gamma} = 4.860 to 5.115 GeV for #eta;|t| [GeV^{2}/c^{4]];Events per 5 MeV^{2}", 380, 0.0, 1.9);
   tEta1->Sumw2();
   GetOutputList()->Add(tEta1);

   dphi_Eta1 = new TH1D("dphi_Eta1", "#Delta#phi from E_{#gamma} = 4.860 to 5.115 GeV for #eta all |t|;#Delta#phi [degree];Events per 2 deg.", 180, -180.0, 180.0);
   dphi_Eta1->Sumw2();
   GetOutputList()->Add(dphi_Eta1);

   tEta2 = new TH1D("tEta2", "|t| from E_{#gamma} = 4.615 to 4.860 GeV for #eta;|t| [GeV^{2}/c^{4]];Events per 5 MeV^{2}", 380, 0.0, 1.9);
   tEta2->Sumw2();
   GetOutputList()->Add(tEta2);

   dphi_Eta2 = new TH1D("dphi_Eta2", "#Delta#phi from E_{#gamma} = 4.615 to 4.860 GeV for #eta all |t|;#Delta#phi [degree];Events per 2 deg.", 180, -180.0, 180.0);
   dphi_Eta2->Sumw2();
   GetOutputList()->Add(dphi_Eta2);

   tEta3 = new TH1D("tEta3", "|t| from E_{#gamma} = 4.358 to 4.615 GeV for #eta;|t| [GeV^{2}/c^{4]];Events per 5 MeV^{2}", 380, 0.0, 1.9);
   tEta3->Sumw2();
   GetOutputList()->Add(tEta3);

   dphi_Eta3 = new TH1D("dphi_Eta3", "#Delta#phi from E_{#gamma} = 4.358 to 4.615 GeV for #eta all |t|;#Delta#phi [degree];Events per 2 deg.", 180, -180.0, 180.0);
   dphi_Eta3->Sumw2();
   GetOutputList()->Add(dphi_Eta3);

   // |t| Bin Range Values
   // Ensure this matches "BinM2g[20][2]" line 219 of "Ebin_allt_Eta0_BkgdFit.C" in STEP2
   double XEta_bins[20][2] = {{0.100, 0.130}, {0.130, 0.160}, {0.160, 0.190}, {0.190, 0.230}, {0.230, 0.270},
   ...
   TString name;
	for (int i = 0; i < 20; i++) {
		name.Form("Eta_dphi_E0_t%d", i);
		title.Form("#Delta#phi from E_{#gamma} = 5.115 to 5.385 GeV for #eta fit, %.3f <= |t| < %.3f;#Delta#phi [degree];Events per 2 deg.", XEta_bins[i][0], XEta_bins[i][1]);
		Eta_dphi_E0_t[i] = new TH1D(name, title, 180, -180.0, 180.0);
		Eta_dphi_E0_t[i]->Sumw2();
		GetOutputList()->Add(Eta_dphi_E0_t[i]);

		name.Form("Eta_dphi_E1_t%d", i);
		title.Form("#Delta#phi from E_{#gamma} = 4.860 to 5.115 GeV for #eta fit, %.3f <= |t| < %.3f;#Delta#phi [degree];Events per 2 deg.", XEta_bins[i][0], XEta_bins[i][1]);
		Eta_dphi_E1_t[i] = new TH1D(name, title, 180, -180.0, 180.0);
		Eta_dphi_E1_t[i]->Sumw2();
		GetOutputList()->Add(Eta_dphi_E1_t[i]);

		name.Form("Eta_dphi_E2_t%d", i);
		title.Form("#Delta#phi from E_{#gamma} = 4.615 to 4.860 GeV for #eta fit, %.3f <= |t| < %.3f;#Delta#phi [degree];Events per 2 deg.", XEta_bins[i][0], XEta_bins[i][1]);
		Eta_dphi_E2_t[i] = new TH1D(name, title, 180, -180.0, 180.0);
		Eta_dphi_E2_t[i]->Sumw2();
		GetOutputList()->Add(Eta_dphi_E2_t[i]);

		name.Form("Eta_dphi_E3_t%d", i);
		title.Form("#Delta#phi from E_{#gamma} = 4.358 to 4.615 GeV for #eta fit, %.3f <= |t| < %.3f;#Delta#phi [degree];Events per 2 deg.", XEta_bins[i][0], XEta_bins[i][1]);
		Eta_dphi_E3_t[i] = new TH1D(name, title, 180, -180.0, 180.0);
		Eta_dphi_E3_t[i]->Sumw2();
		GetOutputList()->Add(Eta_dphi_E3_t[i]);
   }
}


Bool_t TwoG_real2_eta2gp_may_14_2023::Process(Long64_t entry)
{
   ...
   double Eta_bins[20][2] = {{0.100, 0.130}, {0.130, 0.160}, {0.160, 0.190}, {0.190, 0.230}, {0.230, 0.270},
                            {0.270, 0.310}, {0.310, 0.360}, {0.360, 0.410}, {0.410, 0.460}, {0.460, 0.535},
                            {0.535, 0.610}, {0.610, 0.685}, {0.685, 0.785}, {0.785, 0.885}, {0.885, 0.985},
                            {0.985, 1.135}, {1.135, 1.285}, {1.285, 1.460}, {1.460, 1.660}, {1.660, 1.900}};
   */
   ...
   for (int taghit = 0; taghit < ncoin; taghit++) {
      ...
      for (int xp = 0; xp < 20; xp++) {
         if (tabs >= Eta_bins[xp][0] && tabs <= Eta_bins[xp][1]) {
            /* IF THERE ARE MORE THAN 4 PHOTON ENERGY BINS, THEN YOU NEED MODIFY THIS SECTION */
            if (tagErange == 0) {
					tEta0->Fill(tabs, (tag_wgt * Piwgt));
					dphi_Eta0->Fill(del_phi, (tag_wgt * Piwgt));
					Eta_dphi_E0_t[xp]->Fill(del_phi, (tag_wgt * Piwgt));
            }
				else if (tagErange == 1) {
					tEta1->Fill(tabs, (tag_wgt * Piwgt));
					dphi_Eta1->Fill(del_phi, (tag_wgt * Piwgt));
					Eta_dphi_E1_t[xp]->Fill(del_phi, (tag_wgt * Piwgt));
				}
				else if (tagErange == 2) {
					tEta2->Fill(tabs, (tag_wgt * Piwgt));
					dphi_Eta2->Fill(del_phi, (tag_wgt * Piwgt));
					Eta_dphi_E2_t[xp]->Fill(del_phi, (tag_wgt * Piwgt));
				}
				else if (tagErange == 3) {
					tEta3->Fill(tabs, (tag_wgt * Piwgt));
					dphi_Eta3->Fill(del_phi, (tag_wgt * Piwgt));
					Eta_dphi_E3_t[xp]->Fill(del_phi, (tag_wgt * Piwgt));
            }
         }
      }
   }
...
void TwoG_real2_eta2gp_may_14_2023::SlaveTerminate()
{
  
}

void TwoG_real2_eta2gp_may_14_2023::Terminate()
{
   // Write histograms to the output file
   fHistFile = new TFile("TwoG_real2_eta2gp_may_14_2023Analysis.root", "RECREATE");   // Declare output file
...
//////////////////////////////////////////////////////
   ...
   double TwoG_real2_eta2gp_may_14_2023::constrain_angle(double x) {
      double anglex;
      x = remainder(x, 360.0);
      if (x < 0) {
         x += 360.0;
   ...
   double TwoG_real2_eta2gp_may_14_2023::angle_diff(double angle1, double angle2) {
   ...
   double TwoG_real2_eta2gp_may_14_2023::delta_energy(int n_coin, float tagger_energy[30], double particle_ener) {
      double del_ener = 0.0;
