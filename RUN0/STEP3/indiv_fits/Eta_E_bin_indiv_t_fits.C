...
void Eta_E_bin_indiv_t_fits() {

   // Create a Root file to later fill with new histograms for reference in further analysis work
   TFile *StarterFile = new TFile("TwoG_may_14_2023_STEP3.root", "NEW");
   StarterFile->Close();

   // File from STEP2 containing information for calculations & fitting
   TFile *fStep2 = new TFile("/nfs/direct/annex/mcintyre/GitRepo/Radphi_eta2gp/RUN0/STEP2/total_fit/TwoG_may_14_2023_STEP2.root");
   ...
   double Eta_bins[tBinNum][2];
   for (int b1 = 0; b1 < tBinNum; b1++) {
      int bs = b1 + 1;
      Eta_bins[b1][0] = hBinDW->GetBinContent(bs);
      Eta_bins[b1][1] = hBinUP->GetBinContent(bs);

      // For troubleshooting
      //std::cout << "t bin[0] = " << Eta_bins[b1][0] << std::endl;
      //std::cout << "t bin[1] = " << Eta_bins[b1][1] << std::endl;
   }
   ...
   TFile *rootf = new TFile("/nfs/direct/annex/mcintyre/GitRepo/Radphi_eta2gp/RUN0/STEP1/inv_mass/TwoG_may_14_2023Analysis.root");
   ...
   double wR1_Eta[tBinNum];
   double wR2_Eta[tBinNum];
   double wC1_Eta[tBinNum];
   double wC2_Eta[tBinNum];
   double wiEta_s0[tBinNum];
   double wEtaGaus_s0[tBinNum];
   double wiEta_s1[tBinNum];
   double wEtaGaus_s1[tBinNum];
   double wiEta_s2[tBinNum];
   double wEtaGaus_s2[tBinNum];
   double wiEta_b0[tBinNum];
   double wEtaGaus_b0[tBinNum];
   double wiEta_b1[tBinNum];
   double wEtaGaus_b1[tBinNum];
   double wiEta_b2[tBinNum];
   double wEtaGaus_b2[tBinNum];
   ...
   double XEtaDown   = 0.0;
   double XEtaLow    = 0.0;
   double XEtaLower  = 0.0;
   double XEtaHigh   = 0.0;
   double XEtaHigher = 0.0;
   ...
   for (int Echannel = 0; Echannel < NumEChan; Echannel++) {
      ...
      // Use Sideband Subtraction boundary values from STEP2 (over all |t| for a photon energy bin)
		XEtaUp     = RangeUp[Echannel];
		XEtaDown   = RangeDown[Echannel];
		XEtaLow    = RangeLow[Echannel];
		XEtaLower  = RangeLower[Echannel];
		XEtaHigh   = RangeHigh[Echannel];
		XEtaHigher = RangeHigher[Echannel];
      ...
      for (int tchannel = 0; tchannel < tBinNum; tchannel++) {
         ...
         /*
         // For troubleshooting
         std::cout << "XEtaLower = " << XEtaLower * 1000 << std::endl;
         std::cout << "XEtaLow = " << XEtaLow * 1000 << std::endl;
         std::cout << "XEtaDown = " << XEtaDown * 1000 << std::endl;
         std::cout << "XEtaUp = " << XEtaUp * 1000 << std::endl;
         std::cout << "XEtaHigh = " << XEtaHigh * 1000 << std::endl;
         std::cout << "XEtaHigher = " << XEtaHigher * 1000 << std::endl;
         std::cout << std::endl;
         */
			...
			/* S_0 */
			double Sigcount_Eta = fsig2->Integral(XEtaDown, XEtaUp) / binw;

         // These constants make it easier to read the equations below.
         double highX = XEtaUp;
         double lowX  = XEtaDown;
         ////////////////////////////////////////////////////
         /* Integral of Double Gaussian from lowX to highX */
         ////////////////////////////////////////////////////
         double iEta_s0 =  -(sqrt(PI / 2.0) * pow(p[6], 2.0) * p[8]  * TMath::Erf((p[7] - highX)  / (sqrt(2.0) * p[8])))
         ...
         // Partial derivitive of "iEta_s0" wrt p[6]
         double g0s0 =  -sqrt(2.0 * PI) * p[6] * p[8] * TMath::Erf((p[7] - highX) / (sqrt(2.0) * p[8]))
                       + sqrt(2.0 * PI) * p[6] * p[8] * TMath::Erf((p[7] - lowX)  / (sqrt(2.0) * p[8]));

         // Partial derivitive of "iEta_s0" wrt p[7]
         double g1s0 = pow(p[6], 2.0) * (TMath::Power(TMath::E(), (-(pow((p[7] - lowX), 2.0)  / (2.0 * pow(p[8], 2.0))))) - 
                                         TMath::Power(TMath::E(), (-(pow((p[7] - highX), 2.0) / (2.0 * pow(p[8], 2.0))))));

         // Partial derivitive of "iEta_s0" wrt p[8]
         double g2s0 = pow(p[6], 2.0) * ((sqrt(PI / 2.0) * TMath::Erf((p[7] - lowX)  / (sqrt(2.0) * p[8]))) -
         ...
         // Partial derivitive of "iEta_s0" wrt p[9]
         double g3s0 =  -sqrt(2.0 * PI) * p[9] * p[11] * TMath::Erf((p[10] - highX) / (sqrt(2.0) * p[11]))
                       + sqrt(2.0 * PI) * p[9] * p[11] * TMath::Erf((p[10] - lowX)  / (sqrt(2.0) * p[11]));

         // Partial derivitive of "iEta_s0" wrt p[10]
         double g4s0 = pow(p[9], 2.0) * (TMath::Power(TMath::E(), (-(pow((p[10] - lowX), 2.0)  / (2.0 * pow(p[11], 2.0))))) - 
                                         TMath::Power(TMath::E(), (-(pow((p[10] - highX), 2.0) / (2.0 * pow(p[11], 2.0))))));

         // Partial derivitive of "iEta_s0" wrt p[11]
         double g5s0 = pow(p[9], 2.0) * ((sqrt(PI / 2.0) * TMath::Erf((p[10] - lowX)  / (sqrt(2.0) * p[11]))) -
         ...
         double EtaGaus_s0 = sqrt(std::fabs(g0s0 * g0s0 * covMatrix(6, 6) + g1s0 * g1s0 * covMatrix(7, 7)   + g2s0 * g2s0 * covMatrix(8, 8) +
                                            g3s0 * g3s0 * covMatrix(9, 9) + g4s0 * g4s0 * covMatrix(10, 10) + g5s0 * g5s0 * covMatrix(11, 11) +

                                            2.0 * g0s0 * g1s0 * covMatrix(6, 7) + 2.0 * g0s0 * g2s0 * covMatrix(6, 8) +
                                            2.0 * g0s0 * g3s0 * covMatrix(6, 9) + 2.0 * g0s0 * g4s0 * covMatrix(6, 10) + 2.0 * g0s0 * g5s0 * covMatrix(6, 11) +

                                            2.0 * g1s0 * g2s0 * covMatrix(7, 8)  + 2.0 * g1s0 * g3s0 * covMatrix(7, 9) +
                                            2.0 * g1s0 * g4s0 * covMatrix(7, 10) + 2.0 * g1s0 * g5s0 * covMatrix(7, 11) +

                                            2.0 * g2s0 * g3s0 * covMatrix(8, 9) + 2.0 * g2s0 * g4s0 * covMatrix(8, 10) +
                                            2.0 * g2s0 * g5s0 * covMatrix(8, 11) +

                                            2.0 * g3s0 * g4s0 * covMatrix(9, 10) + 2.0 * g3s0 * g5s0 * covMatrix(9, 11) +

                                            2.0 * g4s0 * g5s0 * covMatrix(10, 11)));
         EtaGaus_s0 /= binw;

         std::cout << "****************************************************************************" << std::endl;
         std::cout << "****************************************************************************" << std::endl;
         std::cout << "Eta Integral = " << Sigcount_Eta << " vs. calc -> " << iEta_s0 << std::endl;
         std::cout << "Error = " << EtaGaus_s0 << std::endl;
         std::cout << "****************************************************************************" << std::endl;
         std::cout << "****************************************************************************" << std::endl;
         ...
         */
         ...
         double PercentEtaSig = fsig2->Integral(XEtaDown, XEtaUp) / binw;
         ...
         double PercentEtaSB1 = fsig2->Integral(XEtaLower, XEtaLow) / binw;
         ...
         double PercentEtaSB2 = fsig2->Integral(XEtaHigh, XEtaHigher) / binw;
         ...
         TLine *zline321 = new TLine(XEtaDown, 0.0, XEtaDown, 0.6*normEta);
         zline321->SetLineColor(1);
         zline321->SetLineStyle(9);
         zline321->SetLineWidth(2);
         zline321->Draw("same");

         TLine *zline321a = new TLine(XEtaUp, 0.0, XEtaUp, 0.6*normEta);
         zline321a->SetLineColor(1);
         zline321a->SetLineStyle(9);
         zline321a->SetLineWidth(2);
         zline321a->Draw("same");

         TLine *zline321b = new TLine(XEtaLower, 0.0, XEtaLower, 0.28*normEta);
         zline321b->SetLineColor(6);
         zline321b->SetLineStyle(9);
         zline321b->SetLineWidth(2);
         zline321b->Draw("same");

         TLine *zline321c = new TLine(XEtaHigher, 0.0, XEtaHigher, 0.28*normEta);
         zline321c->SetLineColor(6);
         zline321c->SetLineStyle(9);
         zline321c->SetLineWidth(2);
         zline321c->Draw("same");

         TLine *zline321x = new TLine(XEtaLow, 0.0, XEtaLow, 0.28*normEta);
         zline321x->SetLineColor(6);
         zline321x->SetLineStyle(9);
         zline321x->SetLineWidth(2);
         zline321x->Draw("same");

         TLine *zline321z = new TLine(XEtaHigh, 0.0, XEtaHigh, 0.28*normEta);
         ...
         TString name321;
         name321.Form("SB1 #rightarrow %.1f to %.1f MeV/c^{2}", XEtaLower*1000, XEtaLow*1000);

         TString name321sig;
         name321sig.Form("Signal #rightarrow %.1f to %.1f MeV/c^{2}", XEtaDown*1000, XEtaUp*1000);

         TString name321a;
         name321a.Form("SB2 #rightarrow %.1f to %.1f MeV/c^{2}", XEtaHigh*1000, XEtaHigher*1000);
         ...
         // Signal count in their respective regions
			...
			/* B_1 */
			// Eta SB1 Region
			double a1sb1 = fback->Integral(XEtaLower, XEtaLow) / binw;
			double a2sb1 = fsig1->Integral(XEtaLower, XEtaLow) / binw;
			double a3sb1 = fsig3->Integral(XEtaLower, XEtaLow) / binw;
			double a4sb1 = fsig4->Integral(XEtaLower, XEtaLow) / binw;
			double sb1count_Eta = a1sb1 + a2sb1 + a3sb1 + a4sb1;
         ...
         /////////////////////////////////////////
			double iEta_b1 = sqrt(pow(p[25], 2.0)) * ( p[21] * (highX - lowX) +
                         ...
                         (sqrt(PI / 2.0) * pow(p[18], 2.0) * p[20] * TMath::Erf((p[19]  - highX) / (sqrt(2.0) * p[20]))) -
         ...
         // Etap First gaussian parameters
         // Partial derivitive wrt p[18]
         double g18b1 =  -sqrt(2.0 * PI) * p[18] * p[20] * TMath::Erf((p[19] - highX) / (sqrt(2.0) * p[20]))
                        + sqrt(2.0 * PI) * p[18] * p[20] * TMath::Erf((p[19] - lowX)  / (sqrt(2.0) * p[20]));

         // Partial derivitive wrt p[19]
         double g19b1 = pow(p[18], 2.0) * (TMath::Power(TMath::E(), (-(pow((p[19] - lowX), 2.0)  / (2.0 * pow(p[20], 2.0))))) - 
                                           TMath::Power(TMath::E(), (-(pow((p[19] - highX), 2.0) / (2.0 * pow(p[20], 2.0))))));

         // Partial derivitive wrt p[20]
         double g20b1 = pow(p[18], 2.0) * ((sqrt(PI / 2.0) * TMath::Erf((p[19] - lowX)  / (sqrt(2.0) * p[20]))) -
         ...
         double g17b1 = pow(p[15], 2.0) * ((sqrt(PI / 2.0) * TMath::Erf((p[16] - lowX)  / (sqrt(2.0) * p[17]))) -
			...
			double EtaGaus_b1 = sqrt(std::fabs(g21b1 * g21b1 * covMatrix(21, 21) + g22b1 * g22b1 * covMatrix(22, 22) + g23b1 * g23b1 * covMatrix(23, 23) +
                                  ...
                                  std::fabs(g18b1 * g18b1 * covMatrix(18, 18) + g19b1 * g19b1 * covMatrix(19, 19) + g20b1 * g20b1 * covMatrix(20, 20) +

                                            2.0 * g18b1 * g19b1 * covMatrix(18, 19) + 2.0 * g18b1 * g20b1 * covMatrix(18, 20) +

                                            2.0 * g19b1 * g20b1 * covMatrix(19, 20)) +
         ...
         cout << "Integral = " << sb1count_Eta << endl;
         cout << "Manual = " << iEta_b1 << endl;
         cout << "Error = " << EtaGaus_b1 << endl;
         cout << endl;
			...
			/* B_2 */
			// Eta SB2 Region
			double aa1sb2 = fback->Integral(XEtaHigh, XEtaHigher) / binw;
			double aa2sb2 = fsig1->Integral(XEtaHigh, XEtaHigher) / binw;
			double aa3sb2 = fsig3->Integral(XEtaHigh, XEtaHigher) / binw;
			double aa4sb2 = fsig4->Integral(XEtaHigh, XEtaHigher) / binw;
			double sb2count_Eta = aa1sb2 + aa2sb2 + aa3sb2 + aa4sb2;
         ...
         //////////////////////////////////////////////////
			double iEta_b2 = sqrt(pow(p[25], 2.0)) * ( p[21] * (highX - lowX) +
                                                   (p[22] / 2.0) * (pow(highX, 2.0) - pow(lowX, 2.0)) +
                                                   (p[23] / 3.0) * (pow(highX, 3.0) - pow(lowX, 3.0)) +
                                                   (p[24] / 4.0) * (pow(highX, 4.0) - pow(lowX, 4.0)) ) +
         ...
         double g0b2 =  -sqrt(2.0 * PI) * p[0] * p[2] * TMath::Erf((p[1] - highX) / (sqrt(2.0) * p[2]))
                       + sqrt(2.0 * PI) * p[0] * p[2] * TMath::Erf((p[1] - lowX)  / (sqrt(2.0) * p[2]));

         // Partial derivitive wrt p[1]
         double g1b2 = pow(p[0], 2.0) * (TMath::Power(TMath::E(), (-(pow((p[1] - lowX), 2.0)  / (2.0 * pow(p[2], 2.0))))) - 
                                         TMath::Power(TMath::E(), (-(pow((p[1] - highX), 2.0) / (2.0 * pow(p[2], 2.0))))));

         // Partial derivitive wrt p[2]
         double g2b2 = pow(p[0], 2.0) * ((sqrt(PI / 2.0) * TMath::Erf((p[1] - lowX)  / (sqrt(2.0) * p[2]))) -
         ...
         double g3b2 =  -sqrt(2.0 * PI) * p[3] * p[5] * TMath::Erf((p[4] - highX) / (sqrt(2.0) * p[5]))
                       + sqrt(2.0 * PI) * p[3] * p[5] * TMath::Erf((p[4] - lowX)  / (sqrt(2.0) * p[5]));

         // Partial derivitive wrt p[4]
         double g4b2 = pow(p[3], 2.0) * (TMath::Power(TMath::E(), (-(pow((p[4] - lowX), 2.0)  / (2.0 * pow(p[5], 2.0))))) - 
                                         TMath::Power(TMath::E(), (-(pow((p[4] - highX), 2.0) / (2.0 * pow(p[5], 2.0))))));

         // Partial derivitive wrt p[5]
         double g5b2 = pow(p[3], 2.0) * ((sqrt(PI / 2.0) * TMath::Erf((p[4] - lowX)  / (sqrt(2.0) * p[5]))) -
         ...
         // Etap First gaussian parameters
         // Partial derivitive wrt p[18]
         double g18b2 =  -sqrt(2.0 * PI) * p[18] * p[20] * TMath::Erf((p[19] - highX) / (sqrt(2.0) * p[20]))
                        + sqrt(2.0 * PI) * p[18] * p[20] * TMath::Erf((p[19] - lowX)  / (sqrt(2.0) * p[20]));

         // Partial derivitive wrt p[19]
         double g19b2 = pow(p[18], 2.0) * (TMath::Power(TMath::E(), (-(pow((p[19] - lowX), 2.0)  / (2.0 * pow(p[20], 2.0))))) - 
                                           TMath::Power(TMath::E(), (-(pow((p[19] - highX), 2.0) / (2.0 * pow(p[20], 2.0))))));

         // Partial derivitive wrt p[20]
         double g20b2 = pow(p[18], 2.0) * ((sqrt(PI / 2.0) * TMath::Erf((p[19] - lowX)  / (sqrt(2.0) * p[20]))) -
                                           (sqrt(PI / 2.0) * TMath::Erf((p[19] - highX) / (sqrt(2.0) * p[20]))) -
                                           ((p[19] - lowX)  / p[20]) * TMath::Power(TMath::E(), (-(pow((p[19] - lowX), 2.0))  / (2.0 * pow(p[20], 2.0)))) +
                                           ((p[19] - highX) / p[20]) * TMath::Power(TMath::E(), (-(pow((p[19] - highX), 2.0)) / (2.0 * pow(p[20], 2.0)))));


			double EtaGaus_b2 = sqrt(std::fabs(g21b2 * g21b2 * covMatrix(21, 21) + g22b2 * g22b2 * covMatrix(22, 22) + g23b2 * g23b2 * covMatrix(23, 23) +
                                  ...
                                  std::fabs(g0b2 * g0b2 * covMatrix(0, 0) + g1b2 * g1b2 * covMatrix(1, 1) + g2b2 * g2b2 * covMatrix(2, 2) +
                                            g3b2 * g3b2 * covMatrix(3, 3) + g4b2 * g4b2 * covMatrix(4, 4) + g5b2 * g5b2 * covMatrix(5, 5) +

                                            2.0 * g0b2 * g1b2 * covMatrix(0, 1) + 2.0 * g0b2 * g2b2 * covMatrix(0, 2) +
                                            2.0 * g0b2 * g3b2 * covMatrix(0, 3) + 2.0 * g0b2 * g4b2 * covMatrix(0, 4) + 2.0 * g0b2 * g5b2 * covMatrix(0, 5) +

                                            2.0 * g1b2 * g2b2 * covMatrix(1, 2) + 2.0 * g1b2 * g3b2 * covMatrix(1, 3) +
                                            2.0 * g1b2 * g4b2 * covMatrix(1, 4) + 2.0 * g1b2 * g5b2 * covMatrix(1, 5) +

                                            2.0 * g2b2 * g3b2 * covMatrix(2, 3) + 2.0 * g2b2 * g4b2 * covMatrix(2, 4) +
                                            2.0 * g2b2 * g5b2 * covMatrix(2, 5) +

                                            2.0 * g3b2 * g4b2 * covMatrix(3, 4) + 2.0 * g3b2 * g5b2 * covMatrix(3, 5) +

                                            2.0 * g4b2 * g5b2 * covMatrix(4, 5)) +


                                  std::fabs(g18b2 * g18b2 * covMatrix(18, 18) + g19b2 * g19b2 * covMatrix(19, 19) + g20b2 * g20b2 * covMatrix(20, 20) +

                                            2.0 * g18b2 * g19b2 * covMatrix(18, 19) + 2.0 * g18b2 * g20b2 * covMatrix(18, 20) +

                                            2.0 * g19b2 * g20b2 * covMatrix(19, 20)) +
         ...
         cout << "Integral = " << sb2count_Eta << endl;
         cout << "Manual = " << iEta_b2 << endl;
         cout << "Error = " << EtaGaus_b2 << endl;
         cout << endl;
			...
			/* B_0 */
			// Eta Signal Region
			double Bg_Eta       = fback->Integral(XEtaDown, XEtaUp) / binw;
			double BgEta_Pi     = fsig1->Integral(XEtaDown, XEtaUp) / binw;
			double BgLeak_Eta   = fsig3->Integral(XEtaDown, XEtaUp) / binw;
			double Another_Etap = fsig4->Integral(XEtaDown, XEtaUp) / binw;
			double Bgcount_Eta  = Bg_Eta + BgEta_Pi + BgLeak_Eta + Another_Etap;
         ...
         /////////////////////////////////////////
			...
			double iEta_b0 = sqrt(pow(p[25], 2.0)) * ( p[21] * (highX - lowX) +
                                                   (p[22] / 2.0) * (pow(highX, 2.0) - pow(lowX, 2.0)) +
                                                   (p[23] / 3.0) * (pow(highX, 3.0) - pow(lowX, 3.0)) +
                                                   (p[24] / 4.0) * (pow(highX, 4.0) - pow(lowX, 4.0)) ) +
         ...
         // Etap First gaussian parameters
         // Partial derivitive wrt p[18]
         double g18b0 =  -sqrt(2.0 * PI) * p[18] * p[20] * TMath::Erf((p[19] - highX) / (sqrt(2.0) * p[20]))
                        + sqrt(2.0 * PI) * p[18] * p[20] * TMath::Erf((p[19] - lowX)  / (sqrt(2.0) * p[20]));

         // Partial derivitive wrt p[19]
         double g19b0 = pow(p[18], 2.0) * (TMath::Power(TMath::E(), (-(pow((p[19] - lowX), 2.0)  / (2.0 * pow(p[20], 2.0))))) - 
                                           TMath::Power(TMath::E(), (-(pow((p[19] - highX), 2.0) / (2.0 * pow(p[20], 2.0))))));

         // Partial derivitive wrt p[20]
         double g20b0 = pow(p[18], 2.0) * ((sqrt(PI / 2.0) * TMath::Erf((p[19] - lowX)  / (sqrt(2.0) * p[20]))) -
                                           (sqrt(PI / 2.0) * TMath::Erf((p[19] - highX) / (sqrt(2.0) * p[20]))) -
                                           ((p[19] - lowX)  / p[20]) * TMath::Power(TMath::E(), (-(pow((p[19] - lowX), 2.0))  / (2.0 * pow(p[20], 2.0)))) +
                                           ((p[19] - highX) / p[20]) * TMath::Power(TMath::E(), (-(pow((p[19] - highX), 2.0)) / (2.0 * pow(p[20], 2.0)))));
			...
			double EtaGaus_b0 = sqrt(std::fabs(g21b0 * g21b0 * covMatrix(21, 21) + g22b0 * g22b0 * covMatrix(22, 22) + g23b0 * g23b0 * covMatrix(23, 23) +
                                  ...
                                  std::fabs(g18b0 * g18b0 * covMatrix(18, 18) + g19b0 * g19b0 * covMatrix(19, 19) + g20b0 * g20b0 * covMatrix(20, 20) +

                                            2.0 * g18b0 * g19b0 * covMatrix(18, 19) + 2.0 * g18b0 * g20b0 * covMatrix(18, 20) +

                                            2.0 * g19b0 * g20b0 * covMatrix(19, 20)) +
         ...
         cout << "Integral = " << Bgcount_Eta << endl;
         cout << "Manual = " << iEta_b0 << endl;
         cout << "Error = " << EtaGaus_b0 << endl;
         cout << endl;
			...
			/* S_1 */
			// Eta Signal Region
			double SigSb1_Eta = fsig2->Integral(XEtaLower, XEtaLow) / binw;
         ...
         /* Integral of Double Gaussian from lowX to highX */
         ////////////////////////////////////////////////////
         double iEta_s1 =  -(sqrt(PI / 2.0) * pow(p[6], 2.0) * p[8]  * TMath::Erf((p[7] - highX)  / (sqrt(2.0) * p[8]))) -
                            (sqrt(PI / 2.0) * pow(p[9], 2.0) * p[11] * TMath::Erf((p[10] - highX) / (sqrt(2.0) * p[11]))) +
                            (sqrt(PI / 2.0) * pow(p[6], 2.0) * p[8]  * TMath::Erf((p[7] - lowX)   / (sqrt(2.0) * p[8]))) +
                            (sqrt(PI / 2.0) * pow(p[9], 2.0) * p[11] * TMath::Erf((p[10] - lowX)  / (sqrt(2.0) * p[11])));
         iEta_s1 /= binw;
         ...
         // First gaussian parameters
         // Partial derivitive of "ig" wrt p[6]
         double p6s1 =  -2.0 * sqrt(PI / 2.0) * p[6] * p[8] * TMath::Erf((p[7] - highX) / (sqrt(2.0) * p[8]))
                       + 2.0 * sqrt(PI / 2.0) * p[6] * p[8] * TMath::Erf((p[7] - lowX)  / (sqrt(2.0) * p[8]));

         // Partial derivitive of "ig" wrt p[7]
         double p7s1 =  pow(p[6], 2.0) * (TMath::Power(TMath::E(), (-(pow((p[7] - lowX), 2.0)  / (2.0 * pow(p[8], 2.0))))) - 
                                          TMath::Power(TMath::E(), (-(pow((p[7] - highX), 2.0) / (2.0 * pow(p[8], 2.0))))));

         // Partial derivitive of "ig" wrt p[8]
         double p8s1 =   pow(p[6], 2.0) * ((sqrt(PI / 2.0) * TMath::Erf((p[7] - lowX)  / (sqrt(2.0) * p[8]))) -
                                           (sqrt(PI / 2.0) * TMath::Erf((p[7] - highX) / (sqrt(2.0) * p[8]))) -
                                           (((p[7] - lowX)  * TMath::Power(TMath::E(), (-(pow((p[7] - lowX), 2.0))  / (2.0 * pow(p[8], 2.0))))) / p[8]) +
                                           (((p[7] - highX) * TMath::Power(TMath::E(), (-(pow((p[7] - highX), 2.0)) / (2.0 * pow(p[8], 2.0))))) / p[8]));

         // Second gaussian parameters
         // Partial derivitive of "ig" wrt p[9]
         double p9s1 =  -2.0 * sqrt(PI / 2.0) * p[9] * p[11] * TMath::Erf((p[10] - highX) / (sqrt(2.0) * p[11]))
                       + 2.0 * sqrt(PI / 2.0) * p[9] * p[11] * TMath::Erf((p[10] - lowX)  / (sqrt(2.0) * p[11]));

         // Partial derivitive of "ig" wrt p[10]
         double p10s1 = pow(p[9], 2.0) * (TMath::Power(TMath::E(), (-(pow((p[10] - lowX), 2.0)  / (2.0 * pow(p[11], 2.0))))) - 
                                          TMath::Power(TMath::E(), (-(pow((p[10] - highX), 2.0) / (2.0 * pow(p[11], 2.0))))));

         // Partial derivitive of "ig" wrt p[11]
         double p11s1 = pow(p[9], 2.0) * ((sqrt(PI / 2.0) * TMath::Erf((p[10] - lowX)  / (sqrt(2.0) * p[11]))) -
                                       (sqrt(PI / 2.0) * TMath::Erf((p[10] - highX) / (sqrt(2.0) * p[11]))) -
                                       (((p[10] - lowX)  * TMath::Power(TMath::E(), (-(pow((p[10] - lowX), 2.0))  / (2.0 * pow(p[11], 2.0))))) / p[8]) +
                                       (((p[10] - highX) * TMath::Power(TMath::E(), (-(pow((p[10] - highX), 2.0)) / (2.0 * pow(p[11], 2.0))))) / p[8]));

         double EtaGaus_s1 = sqrt(std::fabs(p6s1 * p6s1 * covMatrix(6, 6) + p7s1 * p7s1 * covMatrix(7, 7) + p8s1 * p8s1 * covMatrix(8, 8) +
                                            p9s1 * p9s1 * covMatrix(9, 9) + p10s1 * p10s1 * covMatrix(10, 10) + p11s1 * p11s1 * covMatrix(11, 11) +

                                            2.0 * p6s1 * p7s1 * covMatrix(6, 7) + 2.0 * p6s1 * p8s1 * covMatrix(6, 8) +
                                            2.0 * p6s1 * p9s1 * covMatrix(6, 9) + 2.0 * p6s1 * p10s1 * covMatrix(6, 10) + 2.0 * p6s1 * p11s1 * covMatrix(6, 11) +

                                            2.0 * p7s1 * p8s1  * covMatrix(7, 8)  + 2.0 * p7s1 * p9s1 * covMatrix(7, 9) +
                                            2.0 * p7s1 * p10s1 * covMatrix(7, 10) + 2.0 * p7s1 * p11s1 * covMatrix(7, 11) +

                                            2.0 * p8s1 * p9s1  * covMatrix(8, 9)  + 2.0 * p8s1 * p10s1 * covMatrix(8, 10) +
                                            2.0 * p8s1 * p11s1 * covMatrix(8, 11) +

                                            2.0 * p9s1 * p10s1 * covMatrix(9, 10) + 2.0 * p9s1 * p11s1 * covMatrix(9, 11) +

                                            2.0 * p10s1 * p11s1 * covMatrix(10, 11)));
         EtaGaus_s1 /= binw;

         cout << " Check S1 Region" << endl;
         cout << "Integral = " << SigSb1_Eta << endl;
         cout << "Manual = " << iEta_s1 << endl;
         cout << "Error = " << EtaGaus_s1 << endl;
         cout << endl;
			...
			/* S_2 */
			// Eta Signal Region
			double SigSb2_Eta = fsig2->Integral(XEtaHigh, XEtaHigher) / binw;
         ...
         /* Integral of Double Gaussian from lowX to highX */
         ////////////////////////////////////////////////////
         double iEta_s2 =  -(sqrt(PI / 2.0) * pow(p[6], 2.0) * p[8]  * TMath::Erf((p[7]  - highX) / (sqrt(2.0) * p[8]))) -
                            (sqrt(PI / 2.0) * pow(p[9], 2.0) * p[11] * TMath::Erf((p[10] - highX) / (sqrt(2.0) * p[11]))) +
                            (sqrt(PI / 2.0) * pow(p[6], 2.0) * p[8]  * TMath::Erf((p[7]  - lowX)  / (sqrt(2.0) * p[8]))) +
                            (sqrt(PI / 2.0) * pow(p[9], 2.0) * p[11] * TMath::Erf((p[10] - lowX)  / (sqrt(2.0) * p[11])));
         iEta_s2 /= binw;
         ...
         // First gaussian parameters
         // Partial derivitive of "ig" wrt p[6]
         double p6s2 =  -2.0 * sqrt(PI / 2.0) * p[6] * p[8] * TMath::Erf((p[7] - highX) / (sqrt(2.0) * p[8]))
                       + 2.0 * sqrt(PI / 2.0) * p[6] * p[8] * TMath::Erf((p[7] - lowX)  / (sqrt(2.0) * p[8]));

         // Partial derivitive of "ig" wrt p[7]
         double p7s2 =  pow(p[6], 2.0) * (TMath::Power(TMath::E(), (-(pow((p[7] - lowX), 2)  / (2.0 * pow(p[8], 2))))) - 
                                          TMath::Power(TMath::E(), (-(pow((p[7] - highX), 2) / (2.0 * pow(p[8], 2))))));

         // Partial derivitive of "ig" wrt p[8]
         double p8s2 =   pow(p[6], 2.0) * ((sqrt(PI / 2.0)  * TMath::Erf((p[7] - lowX)  / (sqrt(2.0) * p[8]))) -
                                           (sqrt(PI / 2.0)  * TMath::Erf((p[7] - highX) / (sqrt(2.0) * p[8]))) -
                                           (((p[7] - lowX)  * TMath::Power(TMath::E(), (-(pow((p[7] - lowX), 2.0))  / (2.0 * pow(p[8], 2.0))))) / p[8]) +
                                           (((p[7] - highX) * TMath::Power(TMath::E(), (-(pow((p[7] - highX), 2.0)) / (2.0 * pow(p[8], 2.0))))) / p[8]));
         ...
         // Second gaussian parameters
         // Partial derivitive of "ig" wrt p[9]
         double p9s2 =  -2.0 * sqrt(PI / 2.0) * p[9] * p[11] * TMath::Erf((p[10] - highX) / (sqrt(2.0) * p[11]))
                       + 2.0 * sqrt(PI / 2.0) * p[9] * p[11] * TMath::Erf((p[10] - lowX)  / (sqrt(2.0) * p[11]));

         // Partial derivitive of "ig" wrt p[10]
         double p10s2 = pow(p[9], 2.0) * (TMath::Power(TMath::E(), (-(pow((p[10] - lowX), 2.0)  / (2.0 * pow(p[11], 2.0))))) - 
                                          TMath::Power(TMath::E(), (-(pow((p[10] - highX), 2.0) / (2.0 * pow(p[11], 2.0))))));

         // Partial derivitive of "ig" wrt p[11]
         double p11s2 = pow(p[9], 2.0) * ((sqrt(PI / 2.0) * TMath::Erf((p[10] - lowX)  / (sqrt(2.0) * p[11]))) -
                                          (sqrt(PI / 2.0) * TMath::Erf((p[10] - highX) / (sqrt(2.0) * p[11]))) -
                                          (((p[10] - lowX)  * TMath::Power(TMath::E(), (-(pow((p[10] - lowX), 2.0))  / (2.0 * pow(p[11], 2.0))))) / p[8]) +
                                          (((p[10] - highX) * TMath::Power(TMath::E(), (-(pow((p[10] - highX), 2.0)) / (2.0 * pow(p[11], 2.0))))) / p[8]));

         double EtaGaus_s2 = sqrt(std::fabs(p6s2 * p6s2 * covMatrix(6, 6) + p7s2 * p7s2 * covMatrix(7, 7) + p8s2 * p8s2 * covMatrix(8, 8) +
                                            p9s2 * p9s2 * covMatrix(9, 9) + p10s2 * p10s2 * covMatrix(10, 10) + p11s2 * p11s2 * covMatrix(11, 11) +

                                            2.0 * p6s2 * p7s2 * covMatrix(6, 7) + 2.0 * p6s2 * p8s2 * covMatrix(6, 8) +
                                            2.0 * p6s2 * p9s2 * covMatrix(6, 9) + 2.0 * p6s2 * p10s2 * covMatrix(6, 10) + 2.0 * p6s2 * p11s2 * covMatrix(6, 11) +

                                            2.0 * p7s2 * p8s2  * covMatrix(7, 8)  + 2.0 * p7s2 * p9s2 * covMatrix(7, 9) +
                                            2.0 * p7s2 * p10s2 * covMatrix(7, 10) + 2.0 * p7s2 * p11s2 * covMatrix(7, 11) +

                                            2.0 * p8s2 * p9s2  * covMatrix(8, 9)  + 2.0 * p8s2 * p10s2 * covMatrix(8, 10) +
                                            2.0 * p8s2 * p11s2 * covMatrix(8, 11) +

                                            2.0 * p9s2 * p10s2 * covMatrix(9, 10) + 2.0 * p9s2 * p11s2 * covMatrix(9, 11) +

                                            2.0 * p10s2 * p11s2 * covMatrix(10, 11)));
         EtaGaus_s2 /= binw;

         cout << " Check S2 Region" << endl;
         cout << "Integral = " << SigSb2_Eta << endl;
         cout << "Manual = " << iEta_s2 << endl;
         cout << "Error = " << EtaGaus_s2 << endl;
         cout << endl;


			/* RATIOS USED TO CALCULATE ACCIDENTAL SUBTRACTION WEIGHTS */
         // Eta Signal - Using manual calculations
			double R1_Eta   = iEta_s1 / iEta_s0;      // R1 = s1/s0
			double R2_Eta   = iEta_s2 / iEta_s0;      // R2 = s2/s0
			double C1_Eta   = iEta_b1 / iEta_b0;      // C1 = b1/b0
			double C2_Eta   = iEta_b2 / iEta_b0;      // C2 = b2/b0

			// Eta Signal - Using integrals
			double R1_Eta2  = SigSb1_Eta / Sigcount_Eta;     // R1 = s1/s0
			double R2_Eta2  = SigSb2_Eta / Sigcount_Eta;     // R2 = s2/s0
			double C1_Eta2  = sb1count_Eta / Bgcount_Eta;    // C1 = b1/b0
			double C2_Eta2  = sb2count_Eta / Bgcount_Eta;    // C2 = b2/b0
         ...
         /*
			...
			// w0 for Eta
			double w_eta_sig = (1.0 + R1_Eta + R2_Eta) / (1.0 - ((R1_Eta + R2_Eta) / (C1_Eta + C2_Eta)));
			// w1 = w2 for Eta
			double w_eta_sb  = -(w_eta_sig / (C1_Eta + C2_Eta));

         double w0_err = sqrt(std::fabs((R1_err * R1_err * (pow((C1_Eta + C2_Eta), 2.0)) * (pow((1.0 + C1_Eta + C2_Eta), 2.0)) +
                                         R2_err * R2_err * (pow((C1_Eta + C2_Eta), 2.0)) * (pow((1.0 + C1_Eta + C2_Eta), 2.0)) +
                                         C1_err * C1_err * (pow((R1_Eta + R2_Eta), 2.0)) * (pow((1.0 + R1_Eta + R2_Eta), 2.0)) +
                                         C2_err * C2_err * (pow((R1_Eta + R2_Eta), 2.0)) * (pow((1.0 + R1_Eta + R2_Eta), 2.0))) /
                                         (pow((R1_Eta + R2_Eta - C1_Eta - C2_Eta), 4.0))));

         double w1_err = sqrt(std::fabs(((R1_err * R1_err + R2_err * R2_err) * (pow((1.0 + C1_Eta + C2_Eta), 2.0)) +
                                         (C1_err * C1_err + C2_err * C2_err) * (pow((1.0 + R1_Eta + R2_Eta), 2.0))) /
                                        (pow((R1_Eta + R2_Eta - C1_Eta - C2_Eta), 4.0))));
         */

			/////////////////// For Eta, using manual calculations ///////////////////
			// w0 for Eta
			double w_eta_sig = (1.0 + R1_Eta + R2_Eta) / (1.0 - ((C1_Eta * R1_Eta + C2_Eta * R2_Eta) / (C1_Eta * C1_Eta + C2_Eta * C2_Eta)));
			// w1 for Eta
			...
			/////////////////// For Eta, using integrals of the fit functions ///////////////////
			// w0 for Eta
			double w_eta2_sig = (1.0 + R1_Eta2 + R2_Eta2) / (1.0 - ((C1_Eta2 * R1_Eta2 + C2_Eta2 * R2_Eta2) / (C1_Eta2 * C1_Eta2 + C2_Eta2 * C2_Eta2)));
			// w1 for Eta
			double w_eta2_sb1 = -(w_eta2_sig) * (C1_Eta2 / (C1_Eta2 * C1_Eta2 + C2_Eta2 * C2_Eta2));
			// w2 for Eta
			double w_eta2_sb2 = -(w_eta2_sig) * (C2_Eta2 / (C1_Eta2 * C1_Eta2 + C2_Eta2 * C2_Eta2));


			//Output file
			ofstream Calcstatus1;
         Calcstatus1.open("Eta_Calculations_check.txt", std::ios_base::app);
         if (Calcstatus1.is_open()) {
            if (tchannel == 0) {
               Calcstatus1 << "____________________________________" << endl; 
               Calcstatus1 << "EtaUp =     " << XEtaUp << endl;
               Calcstatus1 << "EtaDown =   " << XEtaDown << endl;
               Calcstatus1 << "EtaLow =    " << XEtaLow << endl;
               Calcstatus1 << "EtaLower =  " << XEtaLower << endl;
               Calcstatus1 << "EtaHigh =   " << XEtaHigh << endl;
               Calcstatus1 << "EtaHigher = " << XEtaHigher << endl << endl;
            }
				Calcstatus1 << "/////////////////// Weights using manual calculations vs. fit integrals ///////////////////" << endl;
				Calcstatus1 << "w_eta_sig = " << w_eta_sig << " vs. " << w_eta2_sig << endl;
				Calcstatus1 << "w_eta_sb1 = " << w_eta_sb1 << " vs. " << w_eta2_sb1 << endl;
				Calcstatus1 << "w_eta_sb2 = " << w_eta_sb2 << " vs. " << w_eta2_sb2 << endl;
				Calcstatus1 << endl;
            Calcstatus1 << " Check S0 Region" << endl;
            Calcstatus1 << "Integral = " << Sigcount_Eta << endl;
            Calcstatus1 << "Manual = " << iEta_s0 << endl;
            Calcstatus1 << "Error = " << EtaGaus_s0 << endl << endl;

            Calcstatus1 << " Check S1 Region" << endl;
            Calcstatus1 << "Integral = " << SigSb1_Eta << endl;
            Calcstatus1 << "Manual = " << iEta_s1 << endl;
            Calcstatus1 << "Error = " << EtaGaus_s1 << endl << endl;

            Calcstatus1 << " Check S2 Region" << endl;
            Calcstatus1 << "Integral = " << SigSb2_Eta << endl;
            Calcstatus1 << "Manual = " << iEta_s2 << endl;
            Calcstatus1 << "Error = " << EtaGaus_s2 << endl << endl;

            Calcstatus1 << " Check B0 Region" << endl;
            Calcstatus1 << "Integral = " << Bgcount_Eta << endl;
            Calcstatus1 << "Manual = " << iEta_b0 << endl;
            Calcstatus1 << "Error = " << EtaGaus_b0 << endl << endl;

            Calcstatus1 << " Check B1 Region" << endl;
            Calcstatus1 << "Integral = " << sb1count_Eta << endl;
            Calcstatus1 << "Manual = " << iEta_b1 << endl;
            Calcstatus1 << "Error = " << EtaGaus_b1 << endl << endl;

            Calcstatus1 << " Check B2 Region" << endl;
            Calcstatus1 << "Integral = " << sb2count_Eta << endl;
            Calcstatus1 << "Manual = " << iEta_b2 << endl;
            Calcstatus1 << "Error = " << EtaGaus_b2 << endl << endl;
            Calcstatus1 << "---- " << endl;
            Calcstatus1 << "---- " << endl;
            Calcstatus1 << endl;
         }
         else {
            Calcstatus1 << "Unable to open calc. status." << endl;
         }
         Calcstatus1.close();

			/////////////////// For Eta, errors using manual calculations ///////////////////
         double R1_err = std::fabs(R1_Eta) * sqrt(pow((EtaGaus_s0 / iEta_s0), 2.0) + pow((EtaGaus_s1 / iEta_s1), 2.0));

         double R2_err = std::fabs(R2_Eta) * sqrt(pow((EtaGaus_s0 / iEta_s0), 2.0) + pow((EtaGaus_s2 / iEta_s2), 2.0));

         double C1_err = std::fabs(C1_Eta) * sqrt(pow((EtaGaus_b0 / iEta_b0), 2.0) + pow((EtaGaus_b1 / iEta_b1), 2.0));

         double C2_err = std::fabs(C2_Eta) * sqrt(pow((EtaGaus_b0 / iEta_b0), 2.0) + pow((EtaGaus_b2 / iEta_b2), 2.0));

			/////////////////// For Eta, errors using manual calculations ///////////////////
         double w0_err = sqrt(std::fabs((pow(C1_err, 2.0) * pow((2.0 * C1_Eta *(pow(C1_Eta, 2.0) - C1_Eta * R1_Eta + pow(C2_Eta, 2.0) - C2_Eta * R2_Eta) - (2.0 * C1_Eta - R1_Eta) * (pow(C1_Eta, 2.0) + pow(C2_Eta, 2.0))), 2.0) * pow((R2_Eta + R1_Eta + 1.0), 2.0) +
                                         pow(C2_err, 2.0) * pow((2.0 * C2_Eta *(pow(C1_Eta, 2.0) - C1_Eta * R1_Eta + pow(C2_Eta, 2.0) - C2_Eta * R2_Eta) - (2.0 * C2_Eta - R2_Eta) * (pow(C1_Eta, 2.0) + pow(C2_Eta, 2.0))), 2.0) * pow((R2_Eta + R1_Eta + 1.0), 2.0) +
                                         pow(R2_err, 2.0) * pow((pow(C1_Eta, 2.0) + pow(C2_Eta, 2.0)), 2.0) * pow((pow(C1_Eta, 2.0) - (C1_Eta * R1_Eta) + pow(C2_Eta, 2.0) - C2_Eta * R2_Eta + C2_Eta * (R2_Eta + R1_Eta + 1.0)), 2.0) +
                                         pow(R1_err, 2.0) * pow((pow(C1_Eta, 2.0) + pow(C2_Eta, 2.0)), 2.0) * pow((pow(C1_Eta, 2.0) - C1_Eta * R1_Eta + C1_Eta * (R2_Eta + R1_Eta + 1.0) + pow(C2_Eta, 2.0) - C2_Eta * R2_Eta), 2.0))
                                       / pow((pow(C1_Eta, 2.0) - C1_Eta * R1_Eta + pow(C2_Eta, 2.0) - C2_Eta * R2_Eta), 4.0)));

         double w1_err = sqrt(std::fabs((4.0 * pow(w_eta_sig, 2.0) * pow(C1_Eta, 2.0) * pow(C2_Eta, 2.0) * pow(C2_err, 2.0) + pow(w_eta_sig, 2.0) * pow(C1_err, 2.0) * pow((pow(C1_Eta, 2.0) - pow(C2_Eta, 2.0)), 2.0) + pow(C1_Eta, 2.0) * pow(w0_err, 2.0) * pow((pow(C1_Eta, 2.0) + pow(C2_Eta, 2.0)), 2.0)) 
                                      / (pow((pow(C1_Eta, 2.0) + pow(C2_Eta, 2.0)), 4.0))));

         double w2_err = sqrt(std::fabs((4.0 * pow(w_eta_sig, 2.0) * pow(C1_Eta, 2.0) * pow(C2_Eta, 2.0) * pow(C1_err, 2.0) + pow(w_eta_sig, 2.0) * pow(C2_err, 2.0) * pow((pow(C2_Eta, 2.0) - pow(C1_Eta, 2.0)), 2.0) + pow(C2_Eta, 2.0) * pow(w0_err, 2.0) * pow((pow(C1_Eta, 2.0) + pow(C2_Eta, 2.0)), 2.0)) 
                                      / (pow((pow(C1_Eta, 2.0) + pow(C2_Eta, 2.0)), 4.0))));
         ...
         wR1_Eta[tchannel] = R1_Eta;
         wR2_Eta[tchannel] = R2_Eta;
         wC1_Eta[tchannel] = C1_Eta;
         wC2_Eta[tchannel] = C2_Eta;
         wiEta_s0[tchannel] = iEta_s0;
         wEtaGaus_s0[tchannel] = EtaGaus_s0;
         wiEta_s1[tchannel] = iEta_s1;
         wEtaGaus_s1[tchannel] = EtaGaus_s1;
         wiEta_s2[tchannel] = iEta_s2;
         wEtaGaus_s2[tchannel] = EtaGaus_s2;
         wiEta_b0[tchannel] = iEta_b0;
         wEtaGaus_b0[tchannel] = EtaGaus_b0;
         wiEta_b1[tchannel] = iEta_b1;
         wEtaGaus_b1[tchannel] = EtaGaus_b1;
         wiEta_b2[tchannel] = iEta_b2;
         wEtaGaus_b2[tchannel] = EtaGaus_b2;

         // Storing the |t| binned weights in order to add to a .root file for use in later STEP#'s
         w_eta_sigW[tchannel] = w_eta_sig;
         w0_errW[tchannel] = w0_err;

         w1_eta_sbW[tchannel] = w_eta_sb1;
         w1_errW[tchannel] = w1_err;

         w2_eta_sbW[tchannel] = w_eta_sb2;
         ...
         // Eta SB1
         int Eta1bin1 = hfillClone4->FindBin(XEtaLower);
         int Eta1bin2 = hfillClone4->FindBin(XEtaLow);

         // Eta Signal
         int Eta2bin1 = hfillClone5->FindBin(XEtaDown);
         int Eta2bin2 = hfillClone5->FindBin(XEtaUp);

         // Eta SB2
         int Eta3bin1 = hfillClone6->FindBin(XEtaHigh);
         int Eta3bin2 = hfillClone6->FindBin(XEtaHigher);
         ...
         Fitstatus1.open("Eta_fit_status.txt", std::ios_base::app);
         ...
         reset += 1;
			ofstream output;
			output.open("2g_Eta_Results.txt", std::ios_base::app);
			if (output.is_open()) {
            if (reset == 1){
               output << "//-------------------------------//" << std::endl;
            ...
            int RBB = 0;
               ...
               RBB = TC_RangeB[3];
				...
				output << "|t| = " << Eta_bins[tchannel][0] << " to " << Eta_bins[tchannel][1] << " GeV^2" << std::endl;
      ...
      TFile *MyFile = new TFile("TwoG_may_14_2023_STEP3.root", "UPDATE");

      TString hist0Name = Form("h_W0_E%d", Echannel);
      // w0 weight and error
      // bin 1 = w0, bin 2 = error
      TH1D *hEta_W0 = new TH1D(hist0Name, "#eta Signal Region Sideband Subtraction Weight;Weight #pm Error [1^{st} Bin #pm 2^{nd} Bin];Value", 40, 0, 40);
      //hEta_W0->Sumw2();

      // w1 weight and error
      // bin 1 = w1, bin 2 = error
      TString hist1Name = Form("h_W1_E%d", Echannel);
      TH1D *hEta_W1 = new TH1D(hist1Name, "#eta Lower Mass Region Sideband Subtraction Weight;Weight #pm Error [1^{st} Bin #pm 2^{nd} Bin];Value", 40, 0, 40);
      //hEta_W1->Sumw2();

      // w2 weight and error
      // bin 1 = w2, bin 2 = error
      TString hist2Name = Form("h_W2_E%d", Echannel);
      TH1D *hEta_W2 = new TH1D(hist2Name, "#eta Higher Mass Region Sideband Subtraction Weight;Weight #pm Error [1^{st} Bin #pm 2^{nd} Bin];Value", 40, 0, 40);
      //hEta_W2->Sumw2();
      ...
      for (int yurt = 0; yurt < tBinNum; yurt++) {
         ...
         hEta_W0->SetBinContent(bbq, w_eta_sigW[yurt]);
         hEta_W0->SetBinContent(acq, w0_errW[yurt]);

         hEta_W1->SetBinContent(bbq, w1_eta_sbW[yurt]);
         hEta_W1->SetBinContent(acq, w1_errW[yurt]);

         hEta_W2->SetBinContent(bbq, w2_eta_sbW[yurt]);
         hEta_W2->SetBinContent(acq, w2_errW[yurt]);
      }
      gFile = MyFile;
      gDirectory->WriteObject(hEta_W0, hist0Name);
      gDirectory->WriteObject(hEta_W1, hist1Name);
      gDirectory->WriteObject(hEta_W2, hist2Name);
      MyFile->Close();
      ...
      if (FitPar2.is_open()) {
         ...
         for (int iterT = 0; iterT < tBinNum; iterT++) {
            ...
            FitPar2 << "w_eta_sig =  " << w_eta_sigW[iterT] << std::endl;
            FitPar2 << "w0_err    =    " << w0_errW[iterT] << std::endl;
            FitPar2 << "w_eta_sb1 =  " << w1_eta_sbW[iterT] << std::endl;
            FitPar2 << "w1_err    =    " << w1_errW[iterT] << std::endl;
            FitPar2 << "w_eta_sb2 =  " << w2_eta_sbW[iterT] << std::endl;
            FitPar2 << "w2_err    =    " << w2_errW[iterT] << std::endl;

            FitPar2 << "R1_Eta =     " << wR1_Eta[iterT] << std::endl;
            FitPar2 << "R1_err =    " << wR1_err[iterT] << std::endl;
            FitPar2 << "R2_Eta =     " << wR2_Eta[iterT] << std::endl;
            FitPar2 << "R2_err =    " << wR2_err[iterT] << std::endl;
            FitPar2 << "C1_Eta =     " << wC1_Eta[iterT] << std::endl;
            FitPar2 << "C1_err =    " << wC1_err[iterT] << std::endl;
            FitPar2 << "C2_Eta =     " << wC2_Eta[iterT] << std::endl;
            FitPar2 << "C2_err =    " << wC2_err[iterT] << std::endl;

            FitPar2 << "iEta_s0    =    " << wiEta_s0[iterT] << std::endl;
            FitPar2 << "EtaGaus_s0 = " << wEtaGaus_s0[iterT] << std::endl;
            FitPar2 << "iEta_s1    =    " << wiEta_s1[iterT] << std::endl;
            FitPar2 << "EtaGaus_s1 = " << wEtaGaus_s1[iterT] << std::endl;
            FitPar2 << "iEta_s2    =    " << wiEta_s2[iterT] << std::endl;
            FitPar2 << "EtaGaus_s2 = " << wEtaGaus_s2[iterT] << std::endl;
            FitPar2 << "iEta_b0    =    " << wiEta_b0[iterT] << std::endl;
            FitPar2 << "EtaGaus_b0 = " << wEtaGaus_b0[iterT] << std::endl;
            FitPar2 << "iEta_b1    =    " << wiEta_b1[iterT] << std::endl;
            FitPar2 << "EtaGaus_b1 = " << wEtaGaus_b1[iterT] << std::endl;
            FitPar2 << "iEta_b2    =    " << wiEta_b2[iterT] << std::endl;
            FitPar2 << "EtaGaus_b2 = " << wEtaGaus_b2[iterT] << std::endl;
            FitPar2 << "-------------------" << std::endl;
            FitPar2 << std::endl;
         }
         FitPar2 << std::endl;
