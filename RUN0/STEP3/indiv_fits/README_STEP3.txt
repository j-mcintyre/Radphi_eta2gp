...
The sideband subtraction range limits for each photon energy bin (Echannel) are imported fro STEP2:
      RangeLower[Echannel]  = Left sideband lower mass limit
      RangeLow[Echannel]    = Left sideband upper mass limit
      Rangedown[Echannel]   = Signal region lower mass limit
      Rangeup[Echannel]     = Signal region upper mass limit
      RangeHigh[Echannel]   = Right sideband lower mass limit
      RangeHigher[Echannel] = Right sideband upper mass limit
...
They are imported into STEP3 macro via:
      Eta_bins[tchannel][0] = hBinDW->GetBinContent(tchannel + 1);
      Eta_bins[tchannel][1] = hBinUP->GetBinContent(tchannel + 1);
...
3) If the previous step fails, add an IF statement and implement different parameter limits
   ...
   previous fit (2g_E_binned_Eta_fit_allt_*.txt). The IF statement is added to change the
...
4) If all of the above fails to fix things, have a look at the failed TH1D. The single gaussian fit
   ...
   Also, check the text file "2g_E_binned_Eta_fit_allt_*.txt" for the actual fitted values.
